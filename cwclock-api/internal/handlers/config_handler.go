...
// configTokenHeader carries the plaintext API key token to embed in the
// downloaded CLI config/QR code. ApiKeyStore never persists or exposes an
// API key's plaintext once it's been created (see models.ApiKey), so the
// caller - the frontend, right after ApiKeyHandler.Create - is the only
// possible source for it; these endpoints just format it, never look it up.
const configTokenHeader = "X-Config-Token"
...
func (h *ConfigHandler) buildConfigText(r *http.Request, token string) (string, error) {
	userID, _ := middleware.UserIDFromContext(r.Context())

	orgID, err := h.resolveOrgID(r.Context(), userID, r.URL.Query().Get("organization"))
	if err != nil {
		return "", err
	}
