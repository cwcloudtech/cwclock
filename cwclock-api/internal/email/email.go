...
// SendExportJob emails a scheduled export job's generated reports and/or
// attached invoices (see scheduler.ExportJobScheduler) to one "email"
// target's recipients. Unlike SendInvoice there's no organization owner to
// always cc - to/cc are exactly what the job's target configures.
// startTime/endTime are the run's resolved period (see
// scheduler.ParseTimePeriod), shown in full ISO precision in both the
// subject and body - a bare date would be misleading for a period as short
// as an hour. hasReports/hasInvoices (ai-instruct-81) pick the body's
// wording to match what's actually attached: reports only, invoices only,
// or both.
func (s *Sender) SendExportJob(ctx context.Context, to, cc []string, orgID, orgName, jobName string, startTime, endTime time.Time, attachments []Attachment, hasReports, hasInvoices bool) {
	if len(to) == 0 {
		return
	}
	...
	kind := "report(s) and invoice(s)"
	switch {
	case hasReports && !hasInvoices:
		kind = "report(s)"
	case !hasReports && hasInvoices:
		kind = "invoice(s)"
	}

	subject := fmt.Sprintf("%s automated export by %s (%s - %s)", jobName, orgName, startISO, endISO)
	body := template.HTML(fmt.Sprintf(
		`<p>Please find attached the %s generated by the scheduled export job <strong>%s</strong>.</p>`+
			`<p><em>For the period %s - %s</em></p>`,
		kind, template.HTMLEscapeString(jobName), template.HTMLEscapeString(startISO), template.HTMLEscapeString(endISO),
	))
	html, err := renderBody(subject, body, s.logoURL(orgID))
	if err != nil {
		slog.Error("failed to render export job email", "error", err)
		return
	}
	s.send(ctx, strings.Join(to, ","), strings.Join(cc, ","), utils.EMPTY, subject, html, attachments)
}
