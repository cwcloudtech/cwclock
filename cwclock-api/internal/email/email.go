...
// exactly what the job's target configures. startDate/endDate are the run's
// resolved period (see scheduler.ParseTimePeriod), shown in both the
// subject and body so the recipient knows what range the reports cover.
func (s *Sender) SendExportJob(ctx context.Context, to, cc []string, orgID, orgName, jobName, startDate, endDate string, attachments []Attachment) {
	if len(to) == 0 {
		return
	}

	subject := fmt.Sprintf("%s automated export by %s (%s - %s)", jobName, orgName, startDate, endDate)
	body := template.HTML(fmt.Sprintf(
		`<p>Please find attached the report(s) generated by the scheduled export job <strong>%s</strong>.</p>`+
			`<p><em>for the period %s - %s</em></p>`,
		template.HTMLEscapeString(jobName), template.HTMLEscapeString(startDate), template.HTMLEscapeString(endDate),
	))
