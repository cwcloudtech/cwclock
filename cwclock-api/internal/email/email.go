...
// SendExportJob emails a scheduled export job's generated reports (see
// scheduler.ExportJobScheduler) to one "email" target's recipients.
// Unlike SendInvoice there's no organization owner to always cc - to/cc are
// exactly what the job's target configures. startTime/endTime are the run's
// resolved period (see scheduler.ParseTimePeriod), shown in full ISO
// precision in both the subject and body - a bare date would be misleading
// for a period as short as an hour.
func (s *Sender) SendExportJob(ctx context.Context, to, cc []string, orgID, orgName, jobName string, startTime, endTime time.Time, attachments []Attachment) {
	if len(to) == 0 {
		return
	}
	...
	body := template.HTML(fmt.Sprintf(
		`<p>Please find attached the report(s) generated by the scheduled export job <strong>%s</strong>.</p>`+
			`<p><em>For the period %s - %s</em></p>`,
		template.HTMLEscapeString(jobName), template.HTMLEscapeString(startISO), template.HTMLEscapeString(endISO),
	))
