...
// exactly what the job's target configures.
func (s *Sender) SendExportJob(ctx context.Context, to, cc []string, orgID, orgName, jobName string, attachments []Attachment) {
	if len(to) == 0 {
		return
	}

	subject := fmt.Sprintf("%s - %s", orgName, jobName)
	body := template.HTML(fmt.Sprintf(
		`<p>Please find attached the report(s) generated by the scheduled export job <strong>%s</strong>.</p>`,
		template.HTMLEscapeString(jobName),
	))
