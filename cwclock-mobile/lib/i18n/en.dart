// Wording is kept consistent with cwclock-ui's translations/en.js for shared
// concepts (allDay, half, timeTracker.*, errors.*) - not literally imported
// (no shared package between the two apps), just the same phrasing.
const Map<String, dynamic> en = {
  'common': {
    'save': 'Save',
    'cancel': 'Cancel',
    'delete': 'Delete',
    'edit': 'Edit',
    'close': 'Close',
    'loading': 'Loading...',
    'retry': 'Retry',
    'share': 'Share',
    'confirm': 'Confirm',
  },
  'onboarding': {
    'title': 'Connect to CWClock',
    'intro':
        'Scan the QR code from your CWClock API Keys page, or enter your API key manually.',
    'scanQr': 'Scan QR code',
    'enterManually': 'Enter manually',
    'apiUrl': 'API URL',
    'apiKey': 'API key',
    'orgIdOptional': 'Organization ID (optional)',
    'connect': 'Connect',
    'invalidCredentials':
        "Couldn't connect - please check the API URL and API key.",
    'scanHint': 'Point the camera at the QR code from your CWClock API Keys page.',
    'scanFailed':
        "That QR code doesn't look like a CWClock config - please try again or enter manually.",
    'pickOrganization': 'Pick an organization',
    'noOrganizations': 'This account has no organizations yet.',
  },
  'settings': {
    'title': 'Settings',
    'connectedTo': 'Connected to',
    'organization': 'Organization',
    'switchOrganization': 'Switch organization',
    'lightMode': 'Light mode',
    'darkMode': 'Dark mode',
    'upgradeTo': 'Upgrade to v{{version}}',
    'updateFailed': "Couldn't download the update. Please try again.",
    'disconnect': 'Disconnect',
    'disconnectConfirm':
        "Disconnect this device from CWClock? You'll need to scan or enter an API key again.",
    'version': 'Version {{version}}',
  },
  'timeTracker': {
    'title': 'Time tracker',
    'project': 'Project',
    'taskDescription': 'Description',
    'whatAreYouWorkingOn': 'What are you working on?',
    'day': 'Day',
    'startTime': 'Start time',
    'endTime': 'End time',
    'allDay': 'All day',
    'half': 'Half day',
    'start': 'Start',
    'stop': 'Stop',
    'addAllDayRecord': 'Add all-day record',
    'editRecord': 'Edit record',
    'noRecords': 'No time records yet.',
    'deleteRecordTitle': 'Delete time record',
    'deleteRecordBody': 'Delete "{{text}}"? This can\'t be undone.',
    'runningSince': 'Running since {{time}}',
  },
  'reports': {
    'title': 'Reports',
    'type': 'Report type',
    'summary': 'Summary',
    'detailed': 'Detailed',
    'startDate': 'Start date',
    'endDate': 'End date',
    'client': 'Client',
    'project': 'Project',
    'member': 'Member',
    'allClients': 'All clients',
    'allProjects': 'All projects',
    'allMembers': 'All members',
    'generate': 'Generate PDF',
    'generating': 'Generating report...',
  },
  'invoices': {
    'title': 'Invoices',
    'client': 'Client',
    'startDate': 'Start date',
    'endDate': 'End date',
    'preview': 'Preview',
    'generate': 'Generate',
    'generateConfirmTitle': 'Generate invoice',
    'generateConfirmBody':
        'This allocates a real invoice number and saves it. Continue?',
    'previousInvoices': 'Previous invoices',
    'noInvoices': 'No invoices for this client/period yet.',
    'reupload': 'Reupload to external connections',
    'reuploadSuccess': 'Invoice reuploaded.',
    'sendInvoice': 'Send by email',
    'sendInvoiceSuccess': 'Invoice sent.',
    'deleteInvoiceTitle': 'Delete invoice',
    'deleteInvoiceBody': 'Delete invoice {{number}}? This can\'t be undone.',
    'deleteSuccess': 'Invoice deleted.',
    'changeStatus': 'Change status',
    'statusUnpaid': 'Unpaid',
    'statusPaid': 'Paid',
    'statusCanceled': 'Canceled',
    'statusRefunded': 'Refunded',
    'updateStatusSuccess': 'Invoice status updated.',
  },
  'pdf': {
    'title': 'Document',
    'failedToLoad': "Couldn't load the PDF.",
  },
  'management': {
    'title': 'Management',
    'organization': 'Organization',
    'members': 'Members',
    'clients': 'Clients',
    'projects': 'Projects',
    'exportJobs': 'Export jobs',
  },
  'organizations': {
    'profileTitle': 'Organization profile',
    'name': 'Name',
    'accountingEmail': 'Accounting email',
    'address': 'Address',
    'postalCode': 'Postal code',
    'city': 'City',
    'country': 'Country',
    'currency': 'Currency',
    'vatNumber': 'VAT number',
    'identificationNumber': 'Identification number',
    'iban': 'IBAN',
    'bic': 'BIC',
    'ownerOnlyHint': "Only the organization's owner can edit its profile.",
    'membersTitle': 'Members',
    'inviteMember': 'Invite member',
    'email': 'Email',
    'role': 'Role',
    'roleOwner': 'Owner',
    'roleAdmin': 'Admin',
    'roleMember': 'Member',
    'roleReader': 'Reader',
    'changeRole': 'Change role',
    'removeMember': 'Remove',
    'removeMemberTitle': 'Remove member',
    'removeMemberBody': 'Remove {{name}} from this organization?',
    'noMembers': 'No members yet.',
  },
  'clients': {
    'title': 'Clients',
    'addClient': 'Add client',
    'editClient': 'Edit client',
    'name': 'Name',
    'email': 'Email',
    'contactName': 'Contact name',
    'address': 'Address',
    'postalCode': 'Postal code',
    'city': 'City',
    'country': 'Country',
    'vatNumber': 'VAT number',
    'vatRate': 'VAT rate (%)',
    'identificationNumber': 'Identification number',
    'purchaseOrder': 'Purchase order',
    'hoursPerDay': 'Hours per day',
    'dailyRate': 'Daily rate',
    'sendReportsWithInvoice': 'Send reports with invoice',
    'deleteClientTitle': 'Delete client',
    'deleteClientBody': 'Delete "{{name}}"? This also removes its projects.',
    'noClients': 'No clients yet.',
  },
  'projects': {
    'title': 'Projects',
    'addProject': 'Add project',
    'editProject': 'Edit project',
    'name': 'Name',
    'client': 'Client',
    'color': 'Color',
    'dailyRate': 'Daily rate',
    'subdivisions': 'Subdivisions',
    'subdivisionsHint': 'Comma-separated (optional)',
    'deleteProjectTitle': 'Delete project',
    'deleteProjectBody': 'Delete "{{name}}"?',
    'noProjects': 'No projects yet.',
  },
  'exportJobs': {
    'title': 'Export jobs',
    'noJobs': 'No export jobs yet.',
    'runNow': 'Run now',
    'runSuccess': 'Export job started.',
  },
  'errors': {
    'network': 'Something went wrong. Please try again.',
    'notFound': 'Resource not found.',
    'timeEntryFields': 'Please add text, project and day fields.',
    'timeEntryStartEnd':
        'Please add start and end fields, or check all day/half day.',
    'timeEntryAllDayHalf': 'Please check either all day or half day, not both.',
    'ownTimeEntriesOnly': 'You can only manage your own time entries.',
    'invalidInvoiceRequest': 'Please pick a client and a date range.',
    'exportLimitExceeded':
        'This period has too many entries to export. Please narrow the date range.',
    'nameRequired': 'Please fill in the Name field.',
    'countryRequired': 'Please select a country.',
    'invalidEmail': 'Please add a valid email.',
    'invalidCountry': 'Please use a supported country code.',
    'invalidCurrency': 'Please use a supported currency code.',
    'invalidRole': 'Please add a valid role (admin, member or reader).',
    'invalidMemberInvite':
        'Please add a valid email and role (admin, member or reader).',
    'noUserWithEmail': 'No user with this email.',
    'cantRemoveOwner': 'The organization owner cannot be removed.',
    'invalidClientForOrg': 'That client does not belong to this organization.',
  },
};
