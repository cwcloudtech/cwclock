...
var ExportCmd = &cobra.Command{
	...
	Long:  `This command lets you download a summary or detailed time report as a PDF or CSV file over a date range, optionally filtered to one client (every client is included when omitted).`,
	Run: func(cmd *cobra.Command, args []string) {
		err := handlers.HandleExport(orgID, clientID, begin, end, to, reportType, fileFormat, output)
...
func init() {
	...
	ExportCmd.Flags().StringVarP(&clientID, "client", "c", utils.EMPTY, "Client ID filter (optional; every client is included when omitted)")
	...
	ExportCmd.Example = fmt.Sprintf(
		...
		"cwclock export --type detailed --file-format csv --begin now()-30d --end now()",
	)
}
