...
func init() {
	...
	RecordCmd.Flags().StringVar(&begin, "begin", "", "Begin date/time: ISO-8601 or now()/now()-1h/now()-1d style expression")
	RecordCmd.Flags().StringVar(&end, "end", "", "End date/time: ISO-8601 or now()/now()-1h/now()-1d style expression")
	RecordCmd.Flags().StringVar(&from, "from", "", "Alias of --begin")
	RecordCmd.Flags().StringVar(&to, "to", "", "Alias of --end")
	RecordCmd.Flags().StringVarP(&text, "text", "t", "", "Time record description")
	RecordCmd.Flags().StringVarP(&clientID, "client", "c", "", "Client ID (overrides configured client_id)")
	RecordCmd.Flags().StringVarP(&projectID, "project", "p", "", "Project ID (overrides configured project_id)")
	RecordCmd.Flags().StringVarP(&orgID, "org", "o", "", "Organization ID (overrides configured org_id)")
	RecordCmd.Flags().StringVarP(&format, "format", "f", "", "Output format override: pretty|json")
