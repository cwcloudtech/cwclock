...
var CreateCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		} else if utils.IsBlank(effectiveBegin) || utils.IsBlank(effectiveEnd) {
...
func init() {
	...
	CreateCmd.Flags().StringVar(&begin, "begin", utils.EMPTY, "Begin date/time: ISO-8601 (a bare YYYY-MM-DD defaults to 00:00:00), or now()/now()-1h/now()-1d style expression (required)")
